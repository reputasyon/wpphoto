...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  // Type text via chrome.debugger API (triggers real input events that React picks up)
  if (message.action === 'DEBUGGER_TYPE' && sender.tab) {
  ...
  // Trusted mouse click via chrome.debugger API (for WhatsApp elements that ignore synthetic clicks)
  if (message.action === 'DEBUGGER_CLICK' && sender.tab) {
    const tabId = sender.tab.id;
    const debuggee = { tabId };
    const x = message.x;
    const y = message.y;
    ...
    chrome.debugger.attach(debuggee, '1.3', () => {
      ...
      const baseOpts = { x, y, button: 'left', clickCount: 1 };

      chrome.debugger.sendCommand(debuggee, 'Input.dispatchMouseEvent', {
        type: 'mousePressed', ...baseOpts,
      }, () => {
        chrome.debugger.sendCommand(debuggee, 'Input.dispatchMouseEvent', {
          type: 'mouseReleased', ...baseOpts,
        }, () => {
          chrome.debugger.detach(debuggee, () => {
            sendResponse({ success: true });
          });
